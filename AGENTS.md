...
In a **remote Codex session** the git credentials do not have push access to `main`. Direct `git push origin main` fails with HTTP 403. Your **local user** can push to `main` directly.

The correct sequence for remote sessions:

1. Do all work on a feature branch (e.g. `Codex/my-feature`).
